...
st.markdown("Genereer fysisch verklaarbare trainingsdata en omzeil het *Cold Start*-
probleem voordat je echte hardware-defecten hebt gemeten.")
@st.cache_data
def generate_vibration_data(condition, severity, rpm, duration=2.0,
apply_randomness=False):
 sample_rate = 4000
 t = np.linspace(0, duration, int(sample_rate * duration), endpoint=False)
 actueel_rpm = rpm * np.random.normal(1.0, 0.02) if apply_randomness else rpm
 actuele_severity = severity * np.random.normal(1.0, 0.05) if apply_randomness else
severity
...
fase_1x = 2 * np.pi * f_1x * t
 ...
 defect_signaal = np.sin(2 * np.pi * calc_res * t) * impact_envelop *
load_zone_modulatie
 ...
 return t, z_totaal, fft_frequenties, fft_waarden, calc_bpfo, calc_res, actueel_rpm,
actuele_severity
# --- INTERFACE INDELING ---
...
condition = st.sidebar.selectbox("Defect Type", ["Healthy", "Unbalance", "Mechanical
Looseness", "BPFO (Outer Race)"])
t, z_data, freqs, fft_z, bpfo_hz, res_hz, _, _ = generate_vibration_data(condition,
severity, rpm)
col1, col2 = st.columns(2)
with col1:
 st.subheader("Live Tijdsignaal")
 fig_time = go.Figure(go.Scatter(x=t[:800], y=z_data[:800], mode='lines',
line=dict(color='#1f77b4')))
 fig_time.update_layout(xaxis_title="Tijd (s)", yaxis_title="g", height=300,
margin=dict(l=0, r=0, t=30, b=0))
 st.plotly_chart(fig_time, use_container_width=True)
 with col2:
 st.subheader("Live FFT Spectrum")
 fig_fft = go.Figure(go.Scatter(x=freqs, y=fft_z, mode='lines',
line=dict(color='#d62728')))
 ...
 fig_fft.update_layout(xaxis_title="Hz", yaxis_title="Amplitude", height=300,
margin=dict(l=0, r=0, t=30, b=0))
 st.plotly_chart(fig_fft, use_container_width=True)
# --- METRICS & BATCH BILDER ---
st.markdown("---")
st.header("2. Genereer Trainingsdata (Batch Export)")
st.write("Genereer een gebalanceerde dataset met realistische variaties (±2% RPM jitter,
variabele ruisvloer) om overfitting in je Edge AI model te voorkomen.")
...
batch_size = st.slider("Aantal CSV bestanden per conditie", min_value=10, max_value=250,
value=50, step=10)
if st.button(f"Genereer {batch_size} datasets voor '{condition}'"):
 ...
 t_b, z_b, _, _, bpfo_b, res_b, rpm_b, sev_b =
generate_vibration_data(condition, severity, rpm, apply_randomness=True)
 ...
 "file": filename, "condition": condition, "base_rpm": rpm,
"actual_rpm": round(rpm_b, 2),
 "target_severity": severity, "actual_severity": round(sev_b, 3),
"calculated_bpfo_hz": round(bpfo_b, 2), "resonance_hz": round(res_b, 2)
 })
 zip_file.writestr("metadata.json", json.dumps(metadata_list, indent=4))

 st.success("Batch succesvol gegenereerd!")
 st.download_button(label=" Download .ZIP Archief (CSV + JSON)",
data=zip_buffer.getvalue(), file_name=f"dataset_{condition.replace(' ', '_').lower()}_batch.zip", mime="application/zip")
