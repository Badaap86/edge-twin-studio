...
# ============================================================
# DATABASE ENGINE (V15.0 Multi-Tenant SaaS Foundation)
# ============================================================
DB_NAME = 'omega_v15.db'
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS users
                 (id TEXT PRIMARY KEY, username TEXT UNIQUE, password_hash TEXT, api_key TEXT UNIQUE, created_at TEXT)''')
    # Projects Tabel (nu met user_id foreign key)
    c.execute('''CREATE TABLE IF NOT EXISTS projects
                 (id TEXT PRIMARY KEY, user_id TEXT, name TEXT, created_at TEXT, dataset TEXT, settings TEXT)''')
    conn.commit()
    conn.close()
...
# --- PROJECT LOGICA (Gefilterd op User) ---
def save_project(proj_id, user_id, name, dataset_df, settings_dict):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("REPLACE INTO projects (id, user_id, name, created_at, dataset, settings) VALUES (?, ?, ?, ?, ?, ?)",
              (proj_id, user_id, name, str(datetime.datetime.now()), dataset_df.to_json(orient='records'), json.dumps(settings_dict)))
    conn.commit()
    conn.close()
...
def get_user_projects(user_id):
    conn = sqlite3.connect(DB_NAME)
    df = pd.read_sql_query("SELECT id, name, created_at FROM projects WHERE user_id=?", conn, params=(user_id,))
    conn.close()
    return df

# ============================================================
# DSP ENGINE & HARDWARE PROFILER (ONGEWIJZIGD)
# ============================================================
...
def extract_features_from_bytes(file_bytes, filename, sr=16000):
    try:
        ...
        return {"RMS": rms, "Kurtosis": float(kurtosis(sig)), "CrestFactor": float(np.max(np.abs(sig)) / max(rms, 1e-6)), "ZCR": zcr, "SpectralCentroid": cent, "SpectralRolloff": roll, "SpectralFlatness": flat}
...
HARDWARE_PROFILES = {
    ...
    "ARM Cortex-M4F (RAK4631)": {"fft_mult": 0.00015, "feat_mult": 0.4, "inf_mult": 3.0, "target_ram": 256},
    "ARM Cortex-M0+ (RP2040)": {"fft_mult": 0.00080, "feat_mult": 1.5, "inf_mult": 12.0, "target_ram": 264}
}

def get_available_hardware(): return list(HARDWARE_PROFILES.keys())
...
def estimate_edge_load(hw_name, feat_n, sr, duration=1.0):
    ...
    return ram, (fft_n * np.log2(fft_n)) * profile["fft_mult"], feat_n * profile["feat_mult"], profile["inf_mult"]

def calculate_deployment_score(hw_name, latency, ram_kb):
    profile = HARDWARE_PROFILES.get(hw_name, HARDWARE_PROFILES["ARM Cortex-M4F (RAK4631)"])
    base = (max(0, 100 - (latency / 20.0) * 50) * 0.7) + (max(0, 100 - (ram_kb / (profile["target_ram"] / 2)) * 50) * 0.3)
    if profile["fft_mult"] < 0.00010: base += 10 
    elif profile["fft_mult"] < 0.00020: base += 5  
...
def generate_pdf_report(proj_name, num_samples, num_classes, div, bal, sep, top_features, b_dat, best_board):
    ...
    if top_features:
        for f, score in top_features[:5]: pdf.cell(200, 7, txt=f"- {f}: {score:.1f}% impact", ln=True)
    else: pdf.cell(200, 7, txt="Not enough data to calculate feature importance.", ln=True)
    pdf.ln(5)
    
    pdf.set_font("Arial", 'B', 14)
    pdf.cell(200, 10, txt=f"Hardware Recommendation: {best_board}", ln=True)
    pdf.set_font("Arial", '', 11)
    for d in b_dat: pdf.cell(200, 7, txt=f"- {d['Board']}: Score {d['Score']:.0f}% (Lat: {d['Latency']:.1f}ms)", ln=True)
